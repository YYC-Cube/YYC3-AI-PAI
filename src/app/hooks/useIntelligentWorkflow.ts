 ...
 *     if (result.success) {
 *       console.warn('Created workflow:', result.workflow)
 *     }
 *   }
 *
 *   return (
...
export function useIntelligentWorkflow(
  ...
  const {
    autoInitialize = true,
    _autoLoadTemplates = true,
  } = options

  const workflowsMapRef = useRef<Map<string, any> | null>(null)
  const workflowsArrRef = useRef<any[]>([])
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      learnedOptimizations: state.learnedOptimizations,
      stats: state.stats,
  ...
  const workflows = useIntelligentWorkflowStore((state) => {
    if (workflowsMapRef.current === state.workflows) {
      return workflowsArrRef.current
    }
    workflowsMapRef.current = state.workflows
    workflowsArrRef.current = Array.from(state.workflows.values())
    return workflowsArrRef.current
  })
