...
export function useWebGPUInference(
  ...
  const modelsMapRef = useRef<Map<string, AIModel> | null>(null)
  const modelsArrRef = useRef<AIModel[]>([])
  ...
  const {
    ...
    useShallow((state) => ({
      ...
      stats: state.stats,
      initializeWebGPU: state.initializeWebGPU,
      loadModel: state.loadModel,
      unloadModel: state.unloadModel,
      unloadAllModels: state.unloadAllModels,
  ...
  const models = useWebGPUInferenceStore((state) => {
    if (modelsMapRef.current === state.models) {
      return modelsArrRef.current
    }
    modelsMapRef.current = state.models
    modelsArrRef.current = Array.from(state.models.values())
    return modelsArrRef.current
  })
  ...
  useEffect(() => {
    if (autoLoadDefaultModel && webGPUSupported) {
      loadModel(defaultModelId)
        .then(() => setActiveModel(defaultModelId))
        .catch(console.error)
    }
  }, [autoLoadDefaultModel, defaultModelId, webGPUSupported, loadModel, setActiveModel])
