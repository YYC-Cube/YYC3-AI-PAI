 ...
 * ```tsx
 * function MyEditor() {
 *   const editorRef = useRef<monaco._editor.IStandaloneCodeEditor | null>(null)
 *
...
export function useAICompletionProvider(
  editorRef: React.RefObject<Monaco._editor.IStandaloneCodeEditor | null>,
  ...
  const providerRef = useRef<Monaco.languages.CompletionItemProvider | null>(null)
  ...
  // 获取编辑器当前上下文
  const _getContext = useCallback((_editor: Monaco._editor.IStandaloneCodeEditor) => {
  ...
  const parseAIOutput = useCallback(
    (output: string, _input: string): Monaco.languages.CompletionItem[] => {
    ...
    async (input: string): Promise<AICompletionItem[]> => {
      ...
      try {
        ...
        const aiSuggestions: AICompletionItem[] = suggestions.map((suggestion) => ({
          ...
          sortText: 'a' + suggestion.sortText, // AI建议排在前面
        }))
