...
import type { WorkflowNodeType } from '../store/intelligent-workflow-store'
                  ...
                  onClick={() => {
                    if (naturalLanguageInput.trim()) {
                      naturalLanguageToWorkflow(naturalLanguageInput.trim()).then((result) => {
                        if (result.success) {
                          console.warn('Workflow created:', result.workflow)
                  ...
                  {templates.slice(0, 3).map((template) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        console.warn('Created workflow from template:', workflow)
                      }}
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-lg" style={{ background: tk.cardBg, border: `1px solid ${tk.cardBorder}` }}>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <StatItem label="运行时间" value={formatDuration(tick - (executingWorkflow.startedAt || tick))} />
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2 mb-2">
                              ...
                              <div className="flex items-center gap-1 px-1.5 py-0.5 rounded" style={{ background: `${getStatusColor(node.status)}15` }}>
                                <div className="w-1.5 h-1.5 rounded-full" style={{ background: getStatusColor(node.status) }} />
                                <span style={{ fontFamily: tk.fontMono, fontSize: '8px', color: getStatusColor(node.status) }}>
                                  {node.status}
                                </span>
                              </div>
                            </div>
