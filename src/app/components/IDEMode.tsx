...
export function IDEMode({ onSwitchMode, onOpenSettings, onOpenNotifications, onOpenCommandPalette, onOpenGlobalSearch }: {
  ...
  useEffect(() => {
    if (ideStore.activeTabId && ideStore.activeTabId !== selectedFile) setSelectedFileRaw(ideStore.activeTabId);
  }, [ideStore.activeTabId, ideStore, selectedFile]);
  ...
  useEffect(() => { ideStore.setFullscreenPreview(fullscreenPreview); }, [fullscreenPreview, ideStore]);
  useEffect(() => { ideStore.setLeftWidth(resize.leftWidth); }, [resize.leftWidth, ideStore]);
  useEffect(() => { ideStore.setMiddleRatio(resize.middleRatio); }, [resize.middleRatio, ideStore]);
  useEffect(() => { ideStore.setTerminalVisible(terminalVisible); }, [terminalVisible, ideStore]);
  useEffect(() => { ideStore.setTerminalHeight(resize.terminalHeight); }, [resize.terminalHeight, ideStore]);
