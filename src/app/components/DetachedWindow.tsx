...
import { ArrowLeftRight, GripHorizontal, Maximize2, Minus, RotateCcw, X } from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import { useCallback, useRef, useState } from "react";
import { useI18n } from "../i18n/context";
import { PANEL_CONTENT_MAP, usePanelDnD, type DetachedWindow as DetachedWindowType } from "../store/panel-dnd-store";
...
export function DetachedWindowComponent({ window: win, renderContent }: DetachedWindowProps) {
  ...
  }, [win.id, win.position, isMaximized, panelDnD]);
  ...
  const handleMouseDownResize = useCallback((e: React.MouseEvent) => {
    ...
    document.addEventListener("mousemove", onMove);
    document.addEventListener("mouseup", onUp);
  }, [win.id, win.size, isMaximized, panelDnD]);
  ...
  }, [win.id, win.position, win.size, isMaximized, prevBounds, panelDnD]);
  ...
  }, [win.id, win.sourceSlot, win.contentType, panelDnD]);
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        zIndex: win.zIndex,
        background: tokens.panelBg,
        border: `1px solid ${isCrossDropTarget ? tokens.primary
