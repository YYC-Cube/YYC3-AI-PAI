...
export function IDEChatPanel() {
  ...
  // ── MCP tools (local) ──
  const [_mcpRevision, setMcpRevision] = useState(0);
  ...
  const prevMsOpenRef = useRef(msOpen);
  useEffect(() => {
    if (prevMsOpenRef.current && !msOpen) setMcpRevision(r => r + 1);
    prevMsOpenRef.current = msOpen;
  }, [msOpen]);
