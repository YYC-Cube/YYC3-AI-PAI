...
function _getState() {
  // Access internal state via actions — startDrag + cancelDrag to verify
  return panelDnDActions
}
...
describe('Panel DnD Store', () => {
  ...
  it('setSlotContent should change _a specific slot', () => {
...
describe('Panel DnD Window Management', () => {
  ...
  it('detachWindow should create _a new detached window', () => {
  ...
  it('closeWindow should remove _a window', () => {
...
describe('Cross-Window Content Drag', () => {
  ...
  it('swapWindowContents with unknown window should be no-op', () => {
    panelDnDActions.swapWindowContents('nonexistent-_a', 'nonexistent-b')
  })
...
describe('Layout Persistence', () => {
  ...
  it('saveLayout should return _a LayoutSnapshot with name and slots', () => {
    const _layout = panelDnDActions.saveLayout('My Layout')
    expect(_layout.name).toBe('My Layout')
    expect(_layout.id).toMatch(/^layout_/)
    expect(_layout.slots.left).toBe('ai-chat')
    expect(_layout.slots.center).toBe('file-explorer')
    expect(_layout.slots.right).toBe('code-editor')
    expect(_layout.windows).toEqual([])
    expect(_layout.createdAt).toBeGreaterThan(0)
  })
  ...
  it('saveLayout should include detached windows', () => {
    ...
    const _layout = panelDnDActions.saveLayout('With Windows')
    expect(_layout.windows.length).toBe(1)
    expect(_layout.windows[0].contentType).toBe('ai-chat')
    expect(_layout.windows[0].position.x).toBe(200)
    expect(_layout.windows[0].position.y).toBe(150)
  })
  ...
  it('loadLayout should restore slots and windows', () => {
    panelDnDActions.setSlotContent('left', 'terminal')
    panelDnDActions.detachWindow('right')
    const _layout = panelDnDActions.saveLayout('Saved')
    ...
    // Restore
    const result = panelDnDActions.loadLayout(_layout.id)
  ...
  it('deleteLayout should remove _a saved _layout', () => {
    const _layout = panelDnDActions.saveLayout('To Delete')
    expect(panelDnDActions.listLayouts().length).toBe(1)
    panelDnDActions.deleteLayout(_layout.id)
    expect(panelDnDActions.listLayouts().length).toBe(0)
  })

  it('renameLayout should update _layout name and updatedAt', () => {
    const _layout = panelDnDActions.saveLayout('Old Name')
    panelDnDActions.renameLayout(_layout.id, 'New Name')
    const layouts = panelDnDActions.listLayouts()
    expect(layouts[0].name).toBe('New Name')
    expect(layouts[0].updatedAt).toBeGreaterThanOrEqual(_layout.updatedAt)
  })

  it('overwriteLayout should update slots and windows', () => {
    const _layout = panelDnDActions.saveLayout('Overwrite Me')
    panelDnDActions.setSlotContent('center', 'preview')
    panelDnDActions.detachWindow('left')
    panelDnDActions.overwriteLayout(_layout.id)
  ...
  it('autoSaveLayout should create "__autosave__" _layout', () => {
  ...
  it('restoreLastLayout should restore last saved/loaded _layout', () => {
    panelDnDActions.setSlotContent('left', 'terminal')
    const _layout = panelDnDActions.saveLayout('Last Used')
  ...
  it('saveLayout should persist to localStorage under LAYOUTS_KEY', () => {
    ...
    const calls = localStorageMock.setItem.mock.calls.filter(
      (_c: string[]) => _c[0] === 'yyc3-panel-layouts'
    )
  ...
  it('multiple save + delete should maintain correct list', () => {
    const _a = panelDnDActions.saveLayout('A')
    const b = panelDnDActions.saveLayout('B')
    const _c = panelDnDActions.saveLayout('C')
