...
describe('settings-store', () => {
  ...
  describe('Agents', () => {
    it('should have 3 default built-in agents', () => {
      ...
      expect(agents.every((a: any) => a.isBuiltIn)).toBe(true)
    })
    it('should add a custom agent', () => {
      ...
      expect(settingsActions.exportConfig().agents.find((a: any) => a.id === agent.id)).toBeTruthy()
    })
    ...
    it('should remove an agent', () => {
      ...
      expect(settingsActions.exportConfig().agents.find((a: any) => a.id === agent.id)).toBeUndefined()
    })
