...
// Mock IndexedDB with proper IDBDatabase interface
class MockIDBObjectStore {
  private data: Map<string, any> = new Map()
  
  createIndex = vi.fn()
  get = vi.fn().mockImplementation((key) => {
    const request = { result: this.data.get(key), onsuccess: null as any, onerror: null as any }
    setTimeout(() => request.onsuccess?.({ target: request } as any), 0)
    return request
  })
  getAll = vi.fn().mockImplementation(() => {
    const request = { result: Array.from(this.data.values()), onsuccess: null as any, onerror: null as any }
    setTimeout(() => request.onsuccess?.({ target: request } as any), 0)
    return request
  })
  put = vi.fn().mockImplementation((value) => {
    const request = { result: value, onsuccess: null as any, onerror: null as any }
    this.data.set(value.id || Date.now().toString(), value)
    setTimeout(() => request.onsuccess?.({ target: request } as any), 0)
    return request
  })
  clear = vi.fn().mockImplementation(() => {
    const request = { result: undefined, onsuccess: null as any, onerror: null as any }
    this.data.clear()
    setTimeout(() => request.onsuccess?.({ target: request } as any), 0)
    return request
  })
...
class MockIDBDatabase {
  ...
  createObjectStore = vi.fn().mockImplementation(() => new MockIDBObjectStore())
  transaction = vi.fn().mockImplementation(() => new MockIDBTransaction())
  close = vi.fn()
}
...
vi.stubGlobal('indexedDB', mockIndexedDB as any)
