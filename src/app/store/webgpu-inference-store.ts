...
class IndexedDBManager {
  ...
  async init(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        console.warn('[IndexedDB] Database opened successfully')
        resolve(true)
      }
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(this.dbConfig.storeName)) {
          ...
          console.warn('[IndexedDB] Object store created')
        }
      }
    })
  }
  ...
  async saveModel(modelId: string, data: ArrayBuffer | string): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      ...
      request.onsuccess = () => {
        console.warn(`[IndexedDB] Model ${modelId} cached (${(size / 1024 / 1024).toFixed(2)}MB)`)
        resolve(true)
      }

      request.onerror = () => {
        console.error(`[IndexedDB] Failed to cache model ${modelId}`)
        resolve(false)
      }
    })
  }
  ...
  async deleteModel(modelId: string): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      ...
      request.onsuccess = () => {
        console.warn(`[IndexedDB] Model ${modelId} deleted from cache`)
        resolve(true)
      }

      request.onerror = () => {
        console.error(`[IndexedDB] Failed to delete model ${modelId}`)
        resolve(false)
      }
    })
  }
  ...
  private async cleanupOldest(targetSize: number): Promise<void> {
    ...
    console.warn(`[IndexedDB] Cleaned up ${(freedSize / 1024 / 1024).toFixed(2)}MB`)
  }
  ...
  async clearAll(): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      ...
      request.onsuccess = () => {
        console.warn('[IndexedDB] All cache cleared')
        resolve(true)
      }

      request.onerror = () => {
        console.error('[IndexedDB] Failed to clear cache')
        resolve(false)
      }
    })
  }
...
export const useWebGPUInferenceStore = create<WebGPUInferenceStoreState & WebGPUInferenceStoreActions>()(
  immer((set, get) => ({
    ...
    loadModel: async (modelId) => {
      ...
      if (model.loaded) {
        console.warn(`[WebGPU] Model ${modelId} already loaded`)
        return
      }
      ...
      try {
        console.warn(`[WebGPU] Loading model ${modelId}...`)

        // 检查缓存
        const cached = await idbManager.getModel(modelId)
        if (cached) {
          console.warn(`[WebGPU] Loading model ${modelId} from cache...`)
          set((state) => {
            const m = state.models.get(modelId)
          ...
          console.warn(`[WebGPU] Model ${modelId} loaded from cache`)
          return
        }
        ...
        console.warn(`[WebGPU] Model ${modelId} loaded successfully`)
      } catch (error) {
    ...
    unloadModel: (modelId) => {
      ...
      console.warn(`[WebGPU] Model ${modelId} unloaded`)
    },
    ...
    setActiveModel: (modelId) => {
      ...
      console.warn(`[WebGPU] Active model set to ${modelId}`)
    },
    ...
    infer: async (modelId, input) => {
      ...
      try {
        console.warn(`[WebGPU] Starting inference with model ${modelId}...`)
        ...
        console.warn(`[WebGPU] Inference completed in ${inferenceTime.toFixed(0)}ms`)
        return output
      } catch (error) {
    ...
    preloadModel: async (modelId) => {
      ...
      console.warn(`[WebGPU] Preloading model ${modelId}...`)
      await get().loadModel(modelId)
      console.warn(`[WebGPU] Model ${modelId} preloaded`)
    },
