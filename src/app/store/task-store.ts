...
export interface TaskInference {
  task: Omit<Task, 'id' | 'createdAt' | 'updatedAt' | 'isArchived' | 'source' | 'subtasks' | 'dependencies' | 'blocking' | 'relatedFiles'>
  confidence: number
  reasoning: string
  context: string
}
...
function getSnapshot(): TaskStoreState { return { tasks, reminders } }
