...
function _loadPersistedMeta(): PluginMeta[] {
  try {
...
const BUILTIN_PLUGINS: RegisteredPlugin[] = [
  {
    ...
    lifecycle: {
      onActivate: () => console.warn('[Plugin] LocalStorage backend activated'),
      onDeactivate: () => console.warn('[Plugin] LocalStorage backend deactivated'),
    },
    status: 'active',
  },
  {
    ...
    lifecycle: {
      onActivate: () => console.warn('[Plugin] IndexedDB backend activated'),
      onDeactivate: () => console.warn('[Plugin] IndexedDB backend deactivated'),
    },
    status: 'active',
  },
  {
...
export const pluginStoreActions = {
  ...
  registerPlugin(name: string, api: PluginAPI, meta?: Partial<PluginMeta>, lifecycle?: PluginLifecycle): boolean {
    ...
    emit()
    console.warn(`[PluginAPI] Plugin "${name}" registered successfully`)
    return true
  },
