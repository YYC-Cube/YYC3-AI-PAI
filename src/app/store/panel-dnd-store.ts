...
const LAST_LAYOUT_KEY = 'yyc3-panel-last-_layout'
...
function emitChange() {
  ...
  // Trigger debounced auto-save on any state change that affects _layout
  scheduleAutoSave()
}
...
export const panelDnDActions = {
  ...
  loadLayout: (layoutId: string): boolean => {
    const layouts = loadLayouts()
    const _layout = layouts.find(l => l.id === layoutId)
    if (!_layout) return false

    // Close all current windows
    const restoredWindows: DetachedWindow[] = _layout.windows.map((w, i) => ({
    ...
    state = {
      ...state,
      slotContent: { ..._layout.slots },
      detachedWindows: restoredWindows,
      nextZIndex: state.nextZIndex + _layout.windows.length,
      dragSource: null,
      dropTarget: null,
      isDragging: false,
      crossDragSource: null,
