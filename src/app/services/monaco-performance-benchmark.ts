...
export class MonacoPerformanceBenchmark {
  ...
  async testFileOpen(content: string, iterations = 10): Promise<number[]> {
    ...
    this.recordMetric('fileOpen', ...results)
    return results
  }
  ...
  async testCursorMovement(iterations = 100): Promise<number[]> {
    ...
    this.recordMetric('cursorMove', ...results)
    return results
  }
  ...
  async testSyntaxHighlighting(content: string, iterations = 10): Promise<number[]> {
    ...
    this.recordMetric('syntaxHighlight', ...results)
    return results
  }
  ...
  async testCollaboratorCursors(userCount = 10, iterations = 10): Promise<number[]> {
    ...
    this.recordMetric('collabDecorations', ...results)
    return results
  }
  ...
  async testScrolling(scrollLines = 100, iterations = 10): Promise<number[]> {
    ...
    this.recordMetric('scrolling', ...results)
    return results
  }
  ...
  printReport() {
    ...
    console.warn('=== Monaco Editor 性能基准测试报告 ===\n')

    console.warn('📊 文件打开性能:')
    console.warn(`  平均时间: ${report.fileOpenTime.toFixed(2)}ms`)
    console.warn(`  P50: ${report.fileOpenP50.toFixed(2)}ms`)
    console.warn(`  P95: ${report.fileOpenP95.toFixed(2)}ms`)
    console.warn(`  P99: ${report.fileOpenP99.toFixed(2)}ms`)
    console.warn(`  目标: ${PERFORMANCE_TARGETS.fileOpenTime}ms (VS Code)`)
    console.warn(`  达成率: ${(PERFORMANCE_TARGETS.fileOpenTime / report.fileOpenTime * 100).toFixed(1)}%\n`)

    console.warn('⚡ 光标移动性能:')
    console.warn(`  平均时间: ${report.cursorMoveTime.toFixed(2)}ms`)
    console.warn(`  P50: ${report.cursorMoveP50.toFixed(2)}ms`)
    console.warn(`  P95: ${report.cursorMoveP95.toFixed(2)}ms`)
    console.warn(`  P99: ${report.cursorMoveP99.toFixed(2)}ms`)
    console.warn(`  目标: ${PERFORMANCE_TARGETS.cursorMoveTime}ms (VS Code)`)
    console.warn(`  达成率: ${(PERFORMANCE_TARGETS.cursorMoveTime / report.cursorMoveTime * 100).toFixed(1)}%\n`)

    console.warn('🎨 语法高亮性能:')
    console.warn(`  平均时间: ${report.syntaxHighlightTime.toFixed(2)}ms`)
    console.warn(`  P50: ${report.syntaxHighlightP50.toFixed(2)}ms`)
    console.warn(`  P95: ${report.syntaxHighlightP95.toFixed(2)}ms`)
    console.warn(`  P99: ${report.syntaxHighlightP99.toFixed(2)}ms`)
    console.warn(`  目标: ${PERFORMANCE_TARGETS.syntaxHighlightTime}ms (VS Code)`)
    console.warn(`  达成率: ${(PERFORMANCE_TARGETS.syntaxHighlightTime / report.syntaxHighlightTime * 100).toFixed(1)}%\n`)

    console.warn('💾 内存使用:')
    console.warn(`  当前占用: ${report.memoryUsage.toFixed(2)}MB`)
    console.warn(`  目标: ${PERFORMANCE_TARGETS.memoryUsage}MB (VS Code)`)
    console.warn(`  达成率: ${(PERFORMANCE_TARGETS.memoryUsage / report.memoryUsage * 100).toFixed(1)}%`)
    console.warn(`  泄漏率: ${report.memoryLeakRate.toFixed(2)}MB/min\n`)

    console.warn('🎯 总体评分:')
    console.warn(`  综合得分: ${report.overallScore.toFixed(1)}/100`)
    console.warn(`  VS Code对标: ${report.vsCodeScore.toFixed(1)}%\n`)
