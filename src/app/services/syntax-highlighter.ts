...
export class SyntaxHighlighter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.warn(`[SyntaxHighlighter] Initialized in ${initTime.toFixed(2)}ms`)
    } catch (error) {
      console.error('[SyntaxHighlighter] Initialization failed:', error)
      const wrappedError = new Error('Failed to initialize syntax highlighter')
      console.error('[SyntaxHighlighter] Caused by:', error)
      throw wrappedError
    } finally {
  ...
  async highlight(
    ...
    if (this.config.enableCache) {
      ...
      if (cached) {
        console.warn('[SyntaxHighlighter] Cache hit for', language)
  ...
  clearCache(): void {
    this.cache.clear()
    console.warn('[SyntaxHighlighter] Cache cleared')
  }
  ...
  destroy(): void {
    ...
    console.warn('[SyntaxHighlighter] Destroyed')
  }
}
