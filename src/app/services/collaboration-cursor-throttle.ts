  ...
  const throttledUpdate = useThrottleCallback(
    (editor: unknown, decorations: unknown[]) => {
      ...
      if (
        lastArgs &&
        lastArgs[0] === editor &&
        lastArgs[1]?.length === decorations.length &&
        JSON.stringify(lastArgs[1]) === JSON.stringify(decorations)
      ) {
        return // 跳过重复更新
      }

      // 保存参数以便下次比较
      lastUpdateArgsRef.current = [editor, decorations]
      ...
      if (editor && typeof editor === 'object' && 'deltaDecorations' in editor) {
        ...
        decorationsRef.current = monacoEditor.deltaDecorations(oldDecorations, decorations)
      }

      // 性能统计
      console.warn('[CursorThrottle] Decorations updated:', decorations.length, 'cursor(s)')
    },
    config,
  )
 ...
 * @returns 智能更新函数
 */
export function useSmartUpdate<T extends (...args: any[]) => void>(
  updateFn: T,
): T {
